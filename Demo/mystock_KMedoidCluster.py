...
##########################################################################
# read in a sample of ticker price csv, 42 stocks
# split into 3 clusters by distance matrix
# M2 is an array of medoid index, C2 is a dictionary that has an array of index for each cluster
...
for label in C2:
    ...
    print("before sort by distance: ", adist)
    adist.sort(order='dist')
    print("after sort by distance: ", adist)
    sorted_ind = adist['ind']
    print("sorted ticker index: ", sorted_ind)
    print("sorted tickers: ")
    top_ten = 10 if sorted_ind.size > 10 else sorted_ind.size
    for s_idx in range(top_ten):
        print(df.index[sorted_ind[s_idx]], ", ", end='')
    print("\n")
