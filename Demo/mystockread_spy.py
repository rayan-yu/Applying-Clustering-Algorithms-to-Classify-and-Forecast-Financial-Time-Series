...
path_data = 'C:/ry_sci/data/SP500 Daily 12 Yr to 11262018/'
csv_out = 'C:/ry_sci/demo/12y_prices.csv'
sp_csv = os.listdir(path_data+ 'Stocks') # list all price files in the folder
i=0
while i < len(sp_csv):
    print('processing ', sp_csv[i])
    df = pd.read_csv(path_data + 'Stocks/' + sp_csv[i], index_col=['Date'])
    ...
    if i == 0:
        ...
        df.to_csv(csv_out,mode = 'a', header=False)
